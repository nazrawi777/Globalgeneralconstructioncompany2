...
// ============= FINANCIAL DATA =============
// Data is loaded from Django template via window.financialYearsData and window.projectsData
...
function renderLedgerPage() {
  if (!financialYears || financialYears.length === 0) {
    console.warn('No financial years data available');
    return;
  }
  ...
  if (!wrapper) {
    console.warn('Ledger page wrapper not found');
    return;
  }
  
  // Filter projects for this fiscal year
  const yearProjects = projects.filter(p => p.fiscalYear === year.year);
  ...
  setTimeout(() => {
    // Format turnover if not already formatted
    const formattedTurnover = year.formatted || formatCompactCurrency(year.turnover);
      ...
      <div class="page-header">
        <div class="page-header-content">
          ...
          <div>
            ...
            <p class="turnover-value font-mono">${formattedTurnover}</p>
          </div>
        </div>
      </div>
      
      <div class="project-entries no-scrollbar">
        ${yearProjects.length > 0 ? yearProjects.map((project, idx) => `
          <div class="project-entry" style="animation-delay: ${idx * 0.1}s">
            ...
            <div class="project-value-container">
              <p class="project-value font-mono">${project.formattedValue || formatCompactCurrency(project.value)}</p>
              <span class="project-status status-${project.status.toLowerCase()}">${project.status}</span>
            </div>
          </div>
        `).join('') : '<div class="project-entry"><p>No projects for this fiscal year</p></div>'}
      </div>
...
(function() {
  ...
  // Financial data from Django (or fallback)
  const financialYears = window.financialYearsData || [
