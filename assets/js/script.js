...
// ==============================================
// CLEAN CACHE BUSTER - NO META TAGS, NO FOOTER TEXT
// Paste this at the VERY TOP of your main JavaScript file
// ==============================================
(function() {
  'use strict';
  ...
  if (window.cacheBusterProcessed) return;
  window.cacheBusterProcessed = true;
  
  // 3. AUTO-DETECT CURRENT SCRIPT
  let currentScriptName = '';
  if (document.currentScript) {
    const currentScriptSrc = document.currentScript.getAttribute('src');
    if (currentScriptSrc) {
      const parts = currentScriptSrc.split('/');
      currentScriptName = parts[parts.length - 1].split('?')[0];
    }
  }
  
  // 4. UPDATE CSS FILES (SIMPLE)
  ...
  // 5. UPDATE JS FILES (SKIP CURRENT ONE)
  document.querySelectorAll('script[src]').forEach(jsScript => {
    const src = jsScript.getAttribute('src');
    if (src && !src.includes('?')) {
      const parts = src.split('/');
      const filename = parts[parts.length - 1].split('?')[0];
      
      if (currentScriptName && filename === currentScriptName) {
        return; // Skip the current script
      }
      
      jsScript.src = src + '?v=' + timestamp;
    }
  });
